...
class LandNSM5:
    ...
    def __init__(self):
        ...
        try:
            self.ser = serial.Serial(port='COM1', baudrate=38400, bytesize=serial.EIGHTBITS, parity=serial.PARITY_NONE,
    ...
    def stepSlowDistance(self, device, axis, distance):
        ...
        deviceData = (
        [axisNumber, int(dist_4hex[6:], 16), int(dist_4hex[4:6], 16), int(dist_4hex[2:4], 16), int(dist_4hex[:2], 16)])
        res = self.sendCommand(IDcode, nBytes, deviceData, response, len(response))
